...
fn test_alice_and_bob_agree_on_chain_keys_with_kyber_and_frodokexp(
    ...
    let frodokexp_ciphertext = alice_record
        .get_frodokexp_ciphertext()
        .expect("must have session")
        .expect("must have frodokexp ciphertext")
        .clone()
        .into_boxed_slice();

    let frodokexp_tag = alice_record
        .get_frodokexp_tag()
        .expect("must have session")
        .expect("must have frodokexp tag")
        .clone()
        .into_boxed_slice();

    let frodokexp_their_encaps_public_key = {
        let bytes = alice_record
